...
BATCH_SIZE = 16
checkpoint_path = './checkpoints/c3d_fabo_fine_2.pth.tar'
...
def main():
    ...
    optimizer = optim.Adam(params=filter(lambda p: p.requires_grad, net.parameters()), lr=1e-6)
    ...
    # train(trainloader, net, criterion, optimizer, start_epoch=start_epoch, num_epoch=2000, report_step=5)
    test(testloader, net, criterion)
    ...
    #     img = images[0][:,i,:]
    #     arrimg.append(img)
    # npimg = torchvision.utils.make_grid(arrimg).numpy()
